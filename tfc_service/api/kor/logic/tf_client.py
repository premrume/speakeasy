...
def __create_prediction_request__(tokenizer, input_data):
    '''
    Creates prediction request to TensorFlow server for KOR model
    ...
    # Tensorflow magic
    MODEL_NAME  = utils.get_env_var_setting('KOR_MODEL_NAME', settings.DEFAULT_KOR_MODEL_NAME)
    log.debug('using model:'+MODEL_NAME)
    request.model_spec.name = MODEL_NAME

    SIGNATURE_NAME  = utils.get_env_var_setting('KOR_MODEL_SIGNATURE_NAME', settings.DEFAULT_KOR_MODEL_SIGNATURE_NAME)
...
def pad_batch(batch_tokens):
  ...
  log.debug('this is copied from kor_client.py - MAGIC that I did not write')
